...
class Level5 extends React.Component{
    ...
    fetchLevelInfo(){
        return API_LEVEL.getPostLevelInfo(5,(result,status,error) =>{
    ...
    handleSubmitButton(){
        ...
        return API_LEVEL.postLevel5(body,(result,status,error) =>{
            if(result !== null ){
                console.log(result);
                if(status === 202){
                    this.setState({
        ...
        return(
            <div>
                ...
                <CardHeader style = {{fontSize: "20px"}}>
                    <strong> Level5:  {this.state.tableData.levelTitle}</strong>
                </CardHeader>
                ...
                {this.state.resultSuccess && <button type="button" className="btn btn-info m-2"
                    ...
                    {this.state.resultSuccess && <p>Logged in successfully</p>}
