...
class Level7 extends React.Component{
    ...
    fetchLevelInfo(){
        return API_LEVEL.getPostLevelInfo(7,(result,status,error) =>{
    ...
    handleSubmitButton(){
        ...
        if(body.email === "admin" && body.password === "pass"){
            this.setState({
                resultSuccessFinal: true
            })
        }
        
        return API_LEVEL.postLevel7(body,(result,status,error) =>{
            if(result !== null ){
        ...
        return(
            <div>
                ...
                <CardHeader style = {{fontSize: "20px"}}>
                    <strong> Level7:  {this.state.tableData.levelTitle}</strong>
                </CardHeader>
                ...
                {this.state.resultSuccess && <p>Logged In</p>}

                {this.state.resultSuccessFinal && <button type="button" className="btn btn-info m-2"
                                        onClick={()=>{
                                                    this.nextLevel();
